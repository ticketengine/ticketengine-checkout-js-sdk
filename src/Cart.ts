...
export class Cart {
    ...
    public async getOrder(orderId: string, validator?: OrderValidator, retryPolicy: Array<number> = [], forceReload: boolean = false, forceCart: boolean = false): Promise<Order> {
        if(forceReload) {
            await this.fetchOrder(orderId, validator, retryPolicy, forceCart);
        }
